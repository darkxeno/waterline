...
function returnData(results, self) {
...
function returnResults(results, callback, self){
  ...
  findOne: function(criteria, cb, metaContainer) {
    ...
    operations.run(function(err, values) {
      ...
      if (!criteria.joins) {
        ...
        return returnResults(values, cb, self);
      }

      // If the values are already combined, return the results
      if (values.combined) {
        return returnResults(values.cache[self.identity], cb, self);
      }
      ...
      Integrator(values.cache, criteria.joins, primaryKey, function(err, results) {
        ...
        var data = returnData(results, cb, self);

        cb(null, new self._model(data.models[0], data.options));       
