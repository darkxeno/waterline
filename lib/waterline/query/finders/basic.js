...
function returnData(results, self, criteria) {
  ...
  if(criteria.joins){
    return new Joins(criteria.joins, unserializedModels, self.identity, self._schema.schema, self.waterline.collections);
  } else {
    return { models: unserializedModels, options: { showJoins: false } };
  }
}


function returnResults(results, callback, self, criteria, onlyOne){
  
  if (!results) {
    return onlyOne ? callback() : callback(null, []);
  }
  
  var models = [];
  var data = returnData(results, self, criteria);
  
  // If `data.models` is invalid (not an array) return early to avoid getting into trouble.
  if (!data || !data.models || !data.models.forEach) {
    return callback(new Error('Values returned from operations set are not an array...'));
  }   
  
  // Create a model for the top level values
  if(onlyOne){
    callback(null, data.models[0] ? new self._model(data.models[0], data.options) : data.models[0] );
  } else {
...
module.exports = {
  ...
  findOne: function(criteria, cb, metaContainer) {
    ...
    //Forces to return only one value
    criteria.limit = 1;
    ...
    operations.run(function(err, values) {
      ...
      if (!criteria.joins) {
        ...
        return returnResults(values, cb, self, criteria, true);
      }

      // If the values are already combined, return the results
      if (values.combined) {
        return returnResults(values.cache[self.identity], cb, self, criteria, true);
      }
      ...
      Integrator(values.cache, criteria.joins, primaryKey, function(err, results) {
        ...
        returnResults(results, cb, self, criteria, true);        
      });
    });
  },
  ...
  find: function(criteria, options, cb, metaContainer) {
    ...
    operations.run(function(err, values) {
      ...
      if (!criteria.joins) {
        values = values.cache[self.identity];
        return returnResults(values, cb, self, criteria);
      }

      // If the values are already combined, return the results
      if (values.combined) {
        return returnResults(values.cache[self.identity], cb, self, criteria);
      }
      ...
      Integrator(values.cache, criteria.joins, primaryKey, function(err, results) {
        ...
        returnResults(results, cb, self, criteria);        
      });
