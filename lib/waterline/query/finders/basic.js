...
function returnData(results, callback, self) {
  ...
  var data;
  if(criteria.joins){
    data = new Joins(criteria.joins, unserializedModels, self.identity, self._schema.schema, self.waterline.collections);
...
function returnResults(results, cb, self){
  var models = [];
